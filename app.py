...
def record_attendance(emp_id, name):
    ...
    msg = f"Attendance Marked at {timestamp}"
    color = (0.8, 0, 0, 1) if new_status == "out" else (0, 0.8, 0, 1)
...
class DetectApp(App):
    ...
    def build(self):
        ...
        layout = BoxLayout(orientation='vertical', padding=3, spacing=1)

        header_layout = BoxLayout(
            orientation='horizontal',
            size_hint=(1, 0.05),
            padding=[1, 1, 1, 1],   # reduced padding
        ...
        header = Label(
            ...
            halign='left',
            valign='middle',
            size_hint_x=1,         # allow text full width
        )
        header.bind(size=header.setter('text_size'))

        # BIGGER LOGO
        logo = Image(
            source=b_dir + "logo.png",
            size_hint=(None, 1),
        ...
        # ---- Camera Preview Card with PERFECT CLIPPING ----
        from kivy.graphics import StencilPush, StencilUse, StencilPop, StencilUnUse

        self.preview_container = BoxLayout(
            size_hint=(1, 0.60),
            padding=[1, 1, 1, 1],  # perfect 1px gap
            spacing=0
        )
        ...
        with self.preview_container.canvas.before:
            ...
            self.stencil_rect = RoundedRectangle(
                radius=[5],
                pos=self.preview_container.pos,
                size=self.preview_container.size
            )
            StencilUse()
        ...
        with self.preview_container.canvas.after:
            ...
            # Border drawing (outside stencil)
            Color(0.3, 0.3, 0.3, 1)
            self.preview_border = Line(
                rounded_rectangle=[
                    self.preview_container.x,
                    self.preview_container.y,
                    self.preview_container.width,
        ...
        self.preview_container.bind(pos=self.update_preview_card, size=self.update_preview_card)

        self.img_widget = Image(
            size_hint=(1, 1),
            allow_stretch=True,
            keep_ratio=False  # full fit inside clipped region
        )

        self.preview_container.add_widget(self.img_widget)
        ...
        self.info_card = BoxLayout(
            ...
            padding=[1, 10, 1, 10],  # 1px left + 1px right, normal top/bottom
            spacing=5,
            size_hint=(1, None),
            height=110
        )
        ...
        with self.info_card.canvas.before:
            Color(1,1,1,1)
            self.card_bg = RoundedRectangle(radius=[5], pos=self.info_card.pos, size=self.info_card.size)
        with self.info_card.canvas.after:
            Color(0.3,0.3,0.3,1)
            self.card_border = Line(
                rounded_rectangle=[self.info_card.x, self.info_card.y, self.info_card.width, self.info_card.height, 5],
                width=1.2
            )
    ...
    def update_card(self, *args):
        ...
        self.card_border.rounded_rectangle = [
            self.info_card.x, self.info_card.y, self.info_card.width, self.info_card.height, 5
        ]
    def update_preview_card(self, *args):
        self.stencil_rect.pos = self.preview_container.pos
        self.stencil_rect.size = self.preview_container.size

        self.preview_border.rounded_rectangle = [
            self.preview_container.x,
            self.preview_container.y,
            self.preview_container.width,
            self.preview_container.height,
    ...
    def update_frame(self, dt):
        ...
        # ---------- Display frame ----------
        frame_display = cv2.resize(frame, (800, 860))
